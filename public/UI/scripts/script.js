...
var articleService = (function () {
    ...
    var nextIndex;
    var tags = [];
    var artServiceReq = new XMLHttpRequest();
    function cleanArtServiceReq() {
        artServiceReq.removeEventListener('load', fillArticlesWithResponse);
    }
    function gotNextIndexResponse()
    {
        cleanArtServiceReq();
        response = JSON.parse(this.responseText);
        if(response.index)
        {
            nextIndex = response.index;
            console.log("next index from server is:",nextIndex);
        }
    }
    
    function fillArticlesWithResponse() {
      ...
      console.log(articles);
        ...
        artServiceReq.send();
        artServiceReq.addEventListener('load', gotNextIndexResponse);
    }
    function init() {
    ...
    function gotOnAddArticleResponse()
    {
        cleanArtServiceReq();
        ...
        if(response.verdict)
        {
            console.log("Successfultty added article");
        }
        else
        {
            alert("couldn't add article: "+response.explanation);
        }
    }
    function addArticle(article) {
        ...
        if (validateArticle(article)) {
            ...
            artServiceReq.open('POST', "/addArticle");
            artServiceReq.setRequestHeader('content-type', 'application/json');
            const body = JSON.stringify({
                nickname: userName,
                sessionToken: sessionToken,
                article: article
            });
            artServiceReq.send(body);
            artServiceReq.addEventListener('load', gotOnAddArticleResponse);
            nextIndex++;
            saveChanges();
        }
        else return false;
    }
    function gotDeleteResponse()
    {
        artServiceReq.removeEventListener('load',gotDeleteResponse);
    ...
    function removeArticle(id) {
        ...
        artServiceReq.open('DELETE', "/deleteArticle");
        artServiceReq.setRequestHeader('content-type', 'application/json');
        const body = JSON.stringify({
            nickname: userName,
            sessionToken: sessionToken,
        ...
        artServiceReq.send(body);
        artServiceReq.addEventListener('load', gotDeleteResponse);
        // nextIndex--;
        saveChanges();
    }
